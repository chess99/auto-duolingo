...
def check_app_launched(package_name):
    ...
    output = os.popen('adb shell "dumpsys activity activities | grep -i ' + package_name + '"').read()
...
def check_current_app():
    ...
    line = next((line for line in output.splitlines() if 'mFocusedApp' in line), None)
    if line is not None:
